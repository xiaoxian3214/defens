		...
		<view class="teamUl">
			<view class="tuLi" v-for="(item,index) in list" :key='index'>
				...
				<view class="tuliR flexEnd">
					<view class="tuliRli tuliRlicolor1" @click="detail(item)">
						<u-icon name="plus" class="tuliRImg" size="34"></u-icon>
						查看专业队科目
					</view>
					<view class="tuliRli flexCenter1" @click='add(item)' v-if="roleId!=7">
	...
	export default {
		...
		methods: {
			add(item, type) {
				...
				if (item) {
					uni.navigateTo({
						url: './trainMember?teamId=' + item.teamId + '&planId=' + this.id + '&id=' + item
							.trainingList[0].id
					})
				} else {
					uni.navigateTo({
			...
			detail(item) {
				...
				// })
				uni.navigateTo({
					url: './trainingProgramTwo?planId=' + this.id + '&professionalTeamId=' + item.teamId
				})
			},
			// 删除
			dele(item) {
