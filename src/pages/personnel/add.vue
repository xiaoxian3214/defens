<template>
	<view class="add">
		...
		<timePop ref='timePop' @timeDet='timeDet' :isTime='false' :isOld='true'></timePop>
		...
		<view class="detailUl">
			<view class="detailUlLi">
				...
				<view class="detailUlLiR">
					<!-- {{form.name}} -->
					<u-input v-model="form.name" type="text"  placeholder='请输入' class='iulrInput' maxlength='20' />
				</view>
			</view>
			...
			<!-- <view class="detailUlLi" >
				...
				<view class="detailUlLiR">
					<u-input v-model="form.age" type="number" maxlength='3' placeholder='请输入' class='iulrInput' />
				</view>
			</view> -->
		</view>
		<u-gap height="16" bg-color="#FAFAFA"></u-gap>
		...
		<view class="detailUl">
			...
			<view class="detailUlLi">
				...
				<view class="detailUlLiR">
					<!-- {{form.h}} -->
					<u-input v-model="form.workUnit" type="text" placeholder='请输入' class='iulrInput' />
				</view>
			</view>
			<view class="detailUlLi" @click="isRetiredShow=true">
				<view class="detailUlLiL">
				<text class="redTs">*</text>
					是否退役
				</view>
				<view class="detailUlLiR flexCenter">
						<view class="iulRtex" :class="form.isRetired==1||form.isRetired==0?'iulRtexColAct':'iulRtexCol'">
							{{form.isRetired==0?'否':form.isRetired==1?'是':'请选择'}}
						</view>
					<u-select v-model="isRetiredShow" :list="isRetiredList"  @confirm='conPerson'></u-select>
					<image src="../../static/img/right.png" mode="" class="iulRrig"></image>
				</view>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
					...
					eduList:[],
					isRetiredShow:false,
					isRetiredList:[{label:'是',value:1},{label:'否',value:0}],
					detail:'',
					}
		},
		onShow() {
			// console.log(this.$store.state.uploadForm)
			this.detailData=this.$store.state.perFormid
		...
		methods: {
			conPerson(val){
				// console.log(val)
				// this.form.isRetiredName=val[0].label
				this.form.isRetired=val[0].value
			},
			// determine(){
			...
			determine(){
				if(!this.form.name){
					this.$uToast.show('请输入姓名','error')
					return
				}
				if(!this.form.sex){
					this.$uToast.show('请选择性别','error')
					return
				}
				...
				if(!this.form.workUnit){
					this.$uToast.show('请输入工作单位','error')
					return
				}
				let params={
					...this.form
			...
			edit(params){
				this.$Api.playersEdit(params).then(res=>{
					if(res.code==200){
			...
			// 所属专业队
			goExpan(){
				this.$store.commit('perFormChang',this.detailData)
				uni.navigateTo({
