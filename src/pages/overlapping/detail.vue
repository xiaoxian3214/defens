<template>
	<view class="detail">
		...
		<u-gap height="16" bg-color="#FAFAFA"></u-gap>
		<u-button type="primary" class='addDeter' @click='replace'>替换</u-button>
...
<script>
	...
	export default {
		...
		methods: {
			replace(){
				uni.navigateTo({
					url:'./replace?id='+this.id
				})
			},
			detail(){
				this.$Api.crossAuditDetail({id:this.id}).then(res=>{
					if(res.code==200){
						this.form=res.data
						...
						// this.$forceUpdate()
						// console.log(this.form.education,this.form.politicalLandscape)
					}
				})
			},
			// 字典
